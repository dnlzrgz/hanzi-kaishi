...
pub fn create_sentence_model(model_id: i64, model_name: &str) -> Model {
    let custom_css = r#"
            .card {
                --space-1: 4px;
                --space-2: 8px;
...
pub fn build_deck<T: TryIntoNote>(deck: &mut Deck, flashcards: &[T], model: &Model) -> Result<()> {
...
pub fn write_apkg(decks: Vec<Deck>, media_files: &[String], output: &Path) -> Result<()> {
    ...
    let mut package = Package::new(decks, media_files)?;
